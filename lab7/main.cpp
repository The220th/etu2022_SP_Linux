...
int main_child(int fd_file_w, int fd_pipe_r, bool first_second_ch)
{
    ...
    globalvar_fd_r = fd_pipe_r;
    globalvar_fd_w = fd_file_w;
    globalvar_end_flag = 0;
    ...
    {
        pause();
    }while(   !(globalvar_parent_exit_flag == 1 && globalvar_end_flag == 1)   );
...
void process_handler(int sig)
{
    ...
    do
    {
        read_ret = read(globalvar_fd_r, &c, 1);
        if(read_ret <= 0)
        {
            //sleep(1);
